"""OpenPI helpers for RLT Stage 1 offline encoder training.

This module is service-side glue: it may import an external OpenPI checkout,
but it is not imported by the training-side ``vla_rl.policies`` root package.
"""
...
class OpenPIStage1Policy:
    """Frozen OpenPI policy wrapper used only for Stage 1 feature extraction."""
...
class OpenPIStage1Backend:
    """Factory for OpenPI policy and dataloader objects used by RLT Stage 1."""

    def __init__(self, openpi_root: str | Path | None = None) -> None:
    ...
    def load_base_policy(
        ...
        *,
        ...
        return OpenPIStage1Policy(policy=policy, observation_cls=self._model_module.Observation, device=device)
