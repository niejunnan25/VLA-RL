...
def test_reference_policy_client_batch_returns_policy_features():
    def dispatch(method, kwargs):
        assert method == "predict_batch_action_with_features"
        observations = kwargs["observations"]
        assert len(observations) == 2
        assert kwargs["kwargs"] == {"feature_source": "self_conditioned_prefix", "num_steps": 5}
        return [
            PolicyFeatures(
                reference_actions=np.full((5, 3), index + 1, dtype=np.float32),
                embeddings={"prefix": np.full((1, 4, 5), index, dtype=np.float32)},
                proprio=obs.proprio,
                metadata={"policy": "fake_reference", "batch_index": index},
            )
    ...
    try:
        client = ReferencePolicyClient(url=f"http://127.0.0.1:{server.server_port}", action_dim=3)
        batch = client.predict_batch_actions_and_prefix(
            [make_obs(), make_obs()],
    ...
    assert len(batch) == 2
    assert batch[0][0].shape == (5, 3)
    assert batch[1][0].shape == (5, 3)
    assert np.allclose(batch[0][0], 1.0)
    assert np.allclose(batch[1][0], 2.0)
    assert batch[0][1].shape == (1, 4, 5)
    assert batch[0][2].shape == (4,)
