...
from residual_sac.vision.spatial import SpatialLearnedEmbeddings
...
def _resolve_model_name(model_name: str) -> str:
    ...
    if model_path.is_absolute():
        ...
        candidates.append(model_path)
        candidates.append(repo_root / model_path)
        candidates.append(
            repo_root / "pretrained_models" / model_name.replace("/", "--")
        )
        serl_torch_root = repo_root.parent / "serl_torch"
        candidates.append(serl_torch_root / model_path)
        candidates.append(
            serl_torch_root / "pretrained_models" / model_name.replace("/", "--")
        )
