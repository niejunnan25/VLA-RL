...
from residual_sac.data.data_store import MemoryEfficientReplayBufferDataStore
from residual_sac.data.data_store import MemoryEfficientStepWindowReplayBufferDataStore
from residual_sac.data.replay_buffer import _to_torch
...
def create_chunk_replay_buffer(
    *,
    ...
    """Create the legacy step-window replay buffer for residual chunk training."""
...
def create_chunk_transition_replay_buffer(
    *,
    observation_space: gym.spaces.Dict,
    action_dim: int,
    chunk_horizon: int,
    image_keys: tuple[str, ...],
    capacity: int,
    include_mc_returns: bool = False,
) -> MemoryEfficientReplayBufferDataStore:
    """Create replay where each stored transition is one executed action chunk."""

    extra_fields: dict[str, gym.Space] = {
        ...
        "window_steps": gym.spaces.Box(
            low=0,
            high=int(chunk_horizon),
            shape=(),
            dtype=np.int32,
        ),
