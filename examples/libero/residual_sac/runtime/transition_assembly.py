...
from residual_sac.rollout.async_transition_assembly import (
...
def _copy_residual_obs(obs: dict[str, Any]) -> dict[str, np.ndarray]:
    return {str(key): np.asarray(value).copy() for key, value in dict(obs).items()}


def _padded_action_chunk(
    *,
    action_chunk: np.ndarray,
    chunk_horizon: int,
    ...
    if action_array.ndim != 2:
        raise ValueError(
            "chunk-level action_chunk must be 2-D, "
            f"got shape {action_array.shape}"
        )
    executed_steps = int(action_array.shape[0])
    if executed_steps <= 0:
        raise ValueError("chunk-level transition requires at least one executed step")
    horizon = int(chunk_horizon)
    if horizon <= 0:
        raise ValueError(f"chunk_horizon must be positive, got {chunk_horizon}")
    if executed_steps > horizon:
        raise ValueError(
            "executed_steps cannot exceed chunk_horizon for chunk replay: "
            f"executed_steps={executed_steps} chunk_horizon={horizon}"
        )
    padded = np.zeros((horizon, int(action_array.shape[1])), dtype=np.float32)
    action_mask = np.zeros_like(padded, dtype=np.float32)
    padded[:executed_steps] = action_array
    action_mask[:executed_steps] = 1.0
    return padded, action_mask


def assemble_chunk_level_transition(
    *,
    episode_id: int,
    episode_step_start: int,
    residual_obs_before_chunk: dict[str, Any],
    residual_obs_after_chunk: dict[str, Any],
    ...
    if mismatched_lengths:
        raise ValueError(
            "chunk-level transition fields must match executed_actions: "
            f"executed_actions={executed_steps} mismatched={mismatched_lengths}"
        )

    padded_actions, action_mask = _padded_action_chunk(
    ...
    for offset, reward in enumerate(rewards):
        discounted_reward += (discount_value**int(offset)) * float(reward)
    ...
    return [
        {
            "episode_id": int(episode_id),
            "episode_step": int(episode_step_start),
            "observations": _copy_residual_obs(residual_obs_before_chunk),
            "actions": padded_actions,
            "action_mask": action_mask,
            "next_observations": _copy_residual_obs(residual_obs_after_chunk),
            "rewards": np.float32(discounted_reward),
            "masks": np.float32(bootstrap_mask),
            "dones": bool(boundary),
            "window_steps": np.int32(executed_steps),
        }
    ]
...
def infer_chunk_residual_obs_many_with_prompts(
    *,
    ...
    if profile is not None:
        ...
        profile.update(
            {
                "obs_count": int(len(observations)),
                "build_state_sec": 0.0,
                "extract_images_sec": 0.0,
                "build_policy_input_sec": 0.0,
                "postprocess_sec": 0.0,
            }
        )
    ...
    if not callable(infer_many):
        ...
        if profile is not None:
            profile["batched"] = False
            profile["infer_many_wall_sec"] = None
    ...
    for obs in observations:
        ...
        robot_state = build_libero_state(obs)
        if profile is not None:
            profile["build_state_sec"] += time.perf_counter() - start_time

        start_time = time.perf_counter()
        image_observations = extract_libero_images(obs)
        if profile is not None:
            profile["extract_images_sec"] += time.perf_counter() - start_time

        robot_states.append(robot_state)
        image_batches.append(image_observations)
    ...
    if profile is not None:
        profile["build_policy_input_sec"] += time.perf_counter() - start_time

    start_time = time.perf_counter()
    action_chunks, batch_info = infer_many(policy_inputs)
    infer_many_wall_sec = time.perf_counter() - start_time
    if profile is not None:
        profile.update(
            {
                "batched": True,
                "infer_many_wall_sec": float(infer_many_wall_sec),
            }
        )
        if isinstance(batch_info, dict):
            for source_key, target_key in (
                ("e2e_ms", "infer_many_client_e2e_ms"),
                ("server_ms", "infer_many_server_ms"),
                ("policy_ms", "infer_many_policy_ms"),
                ("batch_size", "infer_many_client_batch_size"),
                ("server_batch_size", "infer_many_server_batch_size"),
                ("server_action_dim", "infer_many_server_action_dim"),
                ("used_action_dim", "infer_many_used_action_dim"),
            ):
                if source_key in batch_info and batch_info[source_key] is not None:
                    profile[target_key] = batch_info[source_key]
    ...
    if profile is not None:
        profile["postprocess_sec"] += time.perf_counter() - start_time
    return base_action_chunks, residual_observations
...
def _build_chunk_level_assembly_result(
    *,
    raw: ChunkExecutionRecord,
    chunk_horizon: int,
    discount: float,
) -> AssemblyResult:
    if not raw.residual_obs_before_chunk:
        raise ValueError(
            "chunk replay requires residual_obs_before_chunk from the actor"
        )
    episode_done = bool(raw.chunk_done or raw.chunk_truncated)
    residual_obs_after_chunk = raw.residual_obs_after_chunk
    if residual_obs_after_chunk is None:
        if not episode_done:
            raise ValueError(
                "chunk replay requires residual_obs_after_chunk for non-terminal chunks"
            )
        residual_obs_after_chunk = raw.residual_obs_before_chunk

    transitions = assemble_chunk_level_transition(
        episode_id=int(raw.episode_id),
        episode_step_start=int(raw.episode_step_start),
...
class LiberoTransitionAssembler:
    def __init__(
        ...
        *,
        ...
        image_keys: tuple[str, ...],
        residual_alpha: float,
        transition_granularity: str = "step_window",
        replay_discount: float = 0.99,
    ) -> None:
        self.policy_client = policy_client
        self.chunk_horizon = int(chunk_horizon)
        self.image_keys = tuple(image_keys)
        self.residual_alpha = float(residual_alpha)
        if str(transition_granularity) not in {"step_window", "chunk"}:
            raise ValueError(
                "transition_granularity must be step_window or chunk, "
                f"got {transition_granularity!r}"
            )
        self.transition_granularity = str(transition_granularity)
        self.replay_discount = float(replay_discount)
        self.last_backfill_profile: dict[str, Any] = {}
    ...
    def process_chunk(
        ...
        *,
        ...
        if self.transition_granularity == "chunk":
            del task_prompt
            self.last_backfill_profile = {
                "mode": "chunk_no_backfill",
                "chunk_count": 1,
                "requested_obs_count": 0,
            }
            return _build_chunk_level_assembly_result(
                raw=raw,
                chunk_horizon=self.chunk_horizon,
                discount=self.replay_discount,
            )
    ...
    def process_chunk_batch(
        ...
        *,
        ...
        if self.transition_granularity == "chunk":
            self.last_backfill_profile = {
                "mode": "chunk_no_backfill",
                "chunk_count": int(len(raw_list)),
                "requested_obs_count": 0,
            }
            return [
                _build_chunk_level_assembly_result(
                    raw=raw,
                    chunk_horizon=self.chunk_horizon,
                    discount=self.replay_discount,
                )
        ...
        backfill_profile: dict[str, Any] = {
            "mode": "step_window_batch",
            "chunk_count": int(len(raw_list)),
            "batchable_chunk_count": int(len(batchable_specs)),
            "fallback_chunk_count": int(len(raw_list) - len(batchable_specs)),
            "requested_obs_count": int(len(batched_observations)),
            "collect_obs_sec": float(time.perf_counter() - collect_start_time),
        }

        if batchable_specs:
            infer_profile: dict[str, Any] = {}
            (
            ...
            backfill_profile.update(infer_profile)
            ...
            backfill_profile["assemble_results_sec"] = float(
                time.perf_counter() - assemble_start_time
            )

        self.last_backfill_profile = backfill_profile
...
class LiberoActorTransitionAssembler:
    def __init__(
        ...
        *,
        ...
        self._logger = logger
        transition_granularity = str(cfg.replay.transition_granularity)
        self._sync_assembler = BatchAwareLiberoTransitionAssembler(
            ...
            transition_granularity=transition_granularity,
            replay_discount=float(cfg.sac.discount),
        )
        ...
        if not bool(cfg.backfill_policy.enabled):
            return
        if transition_granularity == "chunk":
            self._logger.info(
                "transition assembler: async backfill disabled for chunk replay"
            )
            return
...
__all__ = [
    ...
    "PrefetchedDecisionObs",
    "ChunkExecutionRecord",
    "assemble_chunk_level_transition",
