...
from residual_sac.common.trainer_transport import SUPPORTED_TRANSPORT_MODES
from residual_sac.common.trainer_transport import TrainerTransportConfig
from residual_sac.common.trainer_transport import validate_transport_mode
from residual_sac.rollout import ProcessorTransportConfig
...
RewardSource = Literal["env", "progress_rpc"]
RewardTransform = Literal[
    "env_only",
    "progress_abs",
    "progress_delta",
    "env_plus_progress",
...
def _optional_float_value(value: Any, field_name: str) -> float | None:
    if value is None:
        return None
...
def _parse_reward_remote_cfg(reward_cfg: Any) -> RewardRemoteConfig:
    ...
    return RewardRemoteConfig(
        ...
        max_message_mb=_positive_int(
            remote_cfg.get("max_message_mb", 256),
        ...
        max_retry_backoff_sec=_nonnegative_float(
            remote_cfg.get("max_retry_backoff_sec", 5.0),
            "reward.remote.max_retry_backoff_sec",
        ),
        compact_transitions=bool(remote_cfg.get("compact_transitions", False)),
    )
...
def _parse_reward_cfg(cfg: DictConfig) -> RewardConfig:
    ...
    transform = _parse_choice(
        reward_cfg.get("transform", default_transform),
        "reward.transform",
        allowed=(
            "env_only",
            "progress_abs",
    ...
    if source == "env" and transform != "env_only":
        raise ValueError(
            "reward.source=env only supports reward.transform=env_only; "
            f"got {transform!r}"
        )
    if source != "env" and transform == "env_only":
        transform = default_transform

    clip_min = _optional_float_value(reward_cfg.get("clip_min", None), "reward.clip_min")
    clip_max = _optional_float_value(reward_cfg.get("clip_max", None), "reward.clip_max")
    ...
    if raw_source == "robodopamine" or raw_source == "robodopamine_rpc":
        ...
        default_name = "progress_rpc"
    raw_name = reward_cfg.get("name", None)
    if source != "env" and str(raw_name) == "sparse":
        raw_name = default_name
    return RewardConfig(
        source=cast(RewardSource, source),
        name=_required_str(default_name if raw_name is None else raw_name, "reward.name"),
        transform=cast(RewardTransform, transform),
        discount=_positive_float(reward_cfg.get("discount", 0.99), "reward.discount"),
        scale=_float_value(reward_cfg.get("scale", 1.0), "reward.scale"),
...
def _parse_replay_cfg(cfg: DictConfig) -> ReplayConfig:
    ...
    transition_granularity = str(
        replay_cfg.get("transition_granularity", "chunk")
    )
    if transition_granularity not in {"step_window", "chunk"}:
        raise ValueError(
            "replay.transition_granularity must be one of "
            f"step_window or chunk, got {transition_granularity!r}"
        )
    return ReplayConfig(
...
def _parse_eval_cfg_block(cfg: DictConfig) -> EvalConfig:
    ...
    return EvalConfig(
        ...
        checkpoint_step=_optional_nonnegative_int(
