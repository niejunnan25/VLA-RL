...
usage() {
  cat <<'EOF'
Usage: examples/libero/rlt/tools/launch_libero90_8tasks_example.sh [options] [-- overrides...]

Starts one RLT tmux session per task by calling examples/libero/rlt/tools/launch_rlt.sh.
...
Options:
  ...
  --actor-gpus CSV             Actor/local-env/policy GPU list.
...
for idx in "${!TASK_ARRAY[@]}"; do
  ...
  cmd=(
    bash "$ROOT/examples/libero/rlt/tools/launch_rlt.sh"
    --config "$CONFIG"
    --session "$session"
    --actor-gpu "$actor_gpu"
