...
usage() {
  cat <<'EOF'
Usage: examples/libero_pld/tools/launch_pld.sh [options] [-- overrides...]
...
tmux new-window -t "$SESSION" -n learner \
  "cd '$ROOT' && CUDA_VISIBLE_DEVICES='$LEARNER_GPU' '$PYTHON_BIN' examples/libero_pld/train.py --role learner --config '$CONFIG' -- ${COMMON_OVERRIDES[*]}"

tmux new-window -t "$SESSION" -n actor \
  "cd '$ROOT' && '$PYTHON_BIN' examples/libero_rlt/tools/wait_for_tcp.py --host 127.0.0.1 --ports '$ENV_PORT' '$POLICY_PORT' '$TRAINER_PORT' --timeout-sec '$WAIT_TIMEOUT_SEC' && CUDA_VISIBLE_DEVICES='$ACTOR_GPU' '$PYTHON_BIN' examples/libero_pld/train.py --role actor --config '$CONFIG' -- ${COMMON_OVERRIDES[*]}"
