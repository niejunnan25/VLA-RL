...
def predict_base_actions(reference_policy, obs: Observation, *, horizon: int, action_dim: int) -> np.ndarray:
    features: PolicyFeatures = reference_policy.extract_features(obs)
    if features.reference_actions is None:
        raise ValueError("reference policy output does not include reference_actions")
    return pld_base_action_prefix(features.reference_actions, horizon=horizon, action_dim=action_dim)
