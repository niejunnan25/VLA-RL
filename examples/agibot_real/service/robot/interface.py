...
class AgiBotRobotNode:
    ...
    def __init__(self, *, hz: float = 20.0) -> None:
        try:
            ...
            no_ros = os.environ.get("AGIBOT_NO_ROS", "").strip()
            if no_ros not in {"1", "true", "TRUE", "yes", "YES"}:
                ensure_repo_local_forwarder()
            ensure_repo_local_a2d_sdk()
            ...
            raise RuntimeError(
                ...
                "Make sure you are using Python 3.10 on the robot machine, "
                "that examples/agibot_real/service/vendor/a2d_sdk is present, and "
                "that AGIBOT_FORWARDER_DIR or AGIBOT_FORWARDER_TAR is set "
                "unless AGIBOT_NO_ROS=1."
            ) from exc
